...
// Duplicamos las tarjetas para que el loop de Embla sea continuo en pantallas anchas.
const carouselClients = [...clients, ...clients, ...clients]

export function Clients() {
  return (
    <section id="clientes" className="py-24 sm:py-32 bg-secondary overflow-hidden">
      ...
      >
        <div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 left-0 z-10 w-8 sm:w-16 bg-gradient-to-r from-secondary to-transparent"
        />
        <div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 right-0 z-10 w-8 sm:w-16 bg-gradient-to-l from-secondary to-transparent"
        />
        ...
        >
          <CarouselContent className="-ml-6 cursor-grab active:cursor-grabbing">
            {carouselClients.map((client, index) => (
              <CarouselItem key={`${client.id}-${index}`} className="pl-6 basis-auto">
